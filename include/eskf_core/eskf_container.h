/*
 * This is the modified version of msf_sortedContainer.h of ETHZ MSF-EKF package,
 * original source code:
 * https://github.com/ethz-asl/ethzasl_msf/blob/master/msf_core/include/msf_core/msf_sortedContainer.h
 *
 * The developer of this modified version: HaoChih, LIN.
 * Email: f44006076@gmail.com
 *
 * The original developer info are as follows:
...
#ifndef ESKF_CONTAINER
#define ESKF_CONTAINER
...
class ESKF_Container {
 ...
 public:
  ...
  ESKF_Container() {
    ...
    invalid->timestamp = INVALID_TIME;
  }
  /**
  ...
  inline typename ListT::iterator Insert(const boost::shared_ptr<T>& value) {
    std::pair<typename ListT::iterator, bool> itpr = stateList.insert(
        std::pair<double, boost::shared_ptr<T> >(value->timestamp, value));
    if (!itpr.second) {
      MSF_WARN_STREAM(
          "Wanted to insert a value to the sorted container at time " <<
          std::fixed << std::setprecision(9) << value->timestamp <<
  ...
  inline typename ListT::iterator GetIteratorAtValue(
    ...
    typename ListT::iterator it = stateList.find(value->timestamp);
    if (it == stateList.end()) {  // There is no value in the map with this time.
      if (warnIfNotExistant)
        MSF_WARN_STREAM(
            ...
            std::fixed << std::setprecision(9) << value->timestamp);
      it = stateList.lower_bound(value->timestamp);
    }
    return it;
  }
  ...
  inline typename ListT::iterator GetIteratorClosest(const double& statetime) {
    ...
    if (fabs(tauPlus->second->timestamp - statetime)
        < fabs(tauMinus->second->timestamp - statetime)) {
      return tauPlus;
    } else {
      return tauMinus;
    }
  ...
  inline boost::shared_ptr<T>& GetClosest(const double& statetime) {
    ...
    if (tauMinus->timestamp == INVALID_TIME) {
      return tauPlus;
    } else if (tauPlus->timestamp == INVALID_TIME) {
      return tauMinus;
    }

    if (fabs(tauPlus->timestamp - statetime) < fabs(tauMinus->timestamp - statetime)) {
      return tauPlus;
    } else {
      return tauMinus;
    }
  ...
  inline void ClearOlderThan(const double age) {
    double newest = GetLast()->timestamp;
    iterator_T it = GetIteratorClosest(newest - age);
    //if (newest - it->second->timestamp < age)
    if(newest - age < GetFirst()->timestamp)
      return;  //there is no state older than time
    if (it->second->timestamp > stateList.begin()->second->timestamp)
  ...
  inline boost::shared_ptr<T> UpdateTime(double timeOld, double timeNew)
    ...
    boost::shared_ptr<T> copy = it->second;
    stateList.erase(it->first);
    copy->timestamp = timeNew;
  ...
  std::string EchoBufferContentTimes() {
    ...
    for (typename ListT::iterator it = GetIteratorBegin();
      ...
      ss << std::fixed << std::setprecision(9) << it->second->timestamp << std::endl;
    }
    return ss.str();
  }
};
